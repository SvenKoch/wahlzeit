...
import org.wahlzeit.model.KeyboardPhotoManager;
...
public abstract class ModelMain extends AbstractMain {
	...
	protected void startUp(String rootDir) throws Exception {
		...
		log.config(LogBuilder.createSystemMessage().addAction("init KeyboardPhotoFactory").toString());
		KeyboardPhotoFactory.initialize();

		log.config(LogBuilder.createSystemMessage().addAction("load KeyboardPhotos").toString());
		KeyboardPhotoManager.getInstance().init();
	}
